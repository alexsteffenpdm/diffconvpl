...
import time
"""
python .\application.py --autorun .\data\generated\star_test.json --no-batch --autosave
"""
...
class BatchRunner():
    def __init__(self, parameters:argparse.Namespace,):
        ...
        self.estimation , self.combinations  = self.approximate_runtime(runtime_per_instance=240)    
        ...
        self.listen_to_dirs: dict[str,Any] =  {
            ...
            "Scene": os.path.join(os.getcwd(),"data","generated","blender_files","scenes")  
        }
    ...
    def approximate_runtime(self,runtime_per_instance: int) -> str:
        ...
        if ans != "y":
            exit()
        return str(timedelta(seconds=runtime_per_instance*len(combinations))), combinations
    ...
    def run(self):
        ...
        for combination in self.combinations:
            ...
            with open(os.path.join(os.getcwd(),"runner_tmp.json"),"w") as runner_config:
            ...
            os.system(f"python .\\application.py --autorun {os.path.join(os.getcwd(),'runner_tmp.json')} --no-batch --autosave --blender-render False")

            end_approximation = datetime.now()
            duration = str(timedelta(seconds=(end_approximation-start_approximation).total_seconds()))
            data_log += f"Startdate: {start_approximation.strftime('%Y-%m-%d_%H-%M-%S')}\n"
        ...
        total_duration = str(timedelta(seconds=(total_end-total_start).total_seconds()))
        data_log = data_log.replace("<TOTAL_TIME>",total_duration)
        data_log = data_log.rstrip()
        with open(os.path.join(os.getcwd(),"data","runner_logs",f"{self.approximation_name}_{timetag}_log.txt"),"w") as logfile:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-full",
        action="store_true",
        help="Enables plotting all max-affine functions"
    )
