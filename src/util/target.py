...
class Target(object):
    ...
    def __init__(self, func: str, parameters: List[str]):
        ...
        self.contains_package: bool = (
            True if "np." in self.func or "torch." in self.func else False
        )
    ...
    def as_lambda(self, package: Optional[str] = None):
        if not self.contains_package:
            ...
            if package is not None:
                if package == "numpy":
                    return eval(
                        f"lambda {','.join(self.parameters)}: {self.func.replace('torch','np') if 'torch' in self.func else self.func}"
                    )
                elif package == "torch":
                    return eval(
                ...
                raise self.UnspecifiedPackageError(
                    f"Target function '{self.func}' requires a package interpretation to be defined please use as_lambda (package=<package_name>) "
                )
