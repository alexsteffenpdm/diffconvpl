...
class Initializer:
    ...
    def __call__(self, entries: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # a vector orthogonal to that function in spatial space
            ...
            raise ValueError(
                f"Evaluations for dimensions > 3 are not defined. (given: {self.dim})"
            )
        return torch.from_numpy(a).type(torch.FloatTensor), torch.from_numpy(b).type(
            torch.FloatTensor
        )
    ...
    def np_array_distance(self, point: np.array) -> float:
        distance = eval(
            f"lambda x: np.sqrt((x - ({point[0]}))**2 + (({self.func_str.split(': ')[1]}) - ({point[-1]}))**2)"
        )
        return np.min(
            np.asarray(
                [
                    ...
                    for xi in np.linspace(
                        self.domain[0], self.domain[1], self.precision
                    )
                ]
            )
        )
    ...
    def torch_distance(self, point: np.array) -> float:
        t = (
            torch.from_numpy(
                np.linspace(self.domain[0], self.domain[1], self.precision)
            )
            .type(torch.FloatTensor)
            .to(torch.device("cuda:0"))
        )
        func = lambda t: torch.sqrt(
            torch.pow(t - point[0], 2) + torch.pow(t**2 - point[1], 2)
        )
        t = func(t)
