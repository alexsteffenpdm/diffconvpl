...
class MemorySharedSubprocess():
    ...
    def _unwrap(self,):
        print(f"Unwrapping shm with name {self.shm.name} to target function {self.target.__name__}")
    ...
    def fork_on(self,data:Dict[str,Any]):
        ...
        if self.shm.size == 0:
            raise ValueError("Cannot fork. Shared memory buffer has not been created until now.")
