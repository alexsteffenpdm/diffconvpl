...
class SDFTriangle2D(SDFGenerator2D):
    ...
    def side_normals(self):
        ...
        for side in self.sides():
            ...
            p4 = [(x4 - self.center[0]), (y4 - self.center[1])] / np.linalg.norm(
                [(x4 - self.center[0]), (y4 - self.center[1])]
            )
            normals.append(p4)
...
if __name__ == "__main__":
    ...
    assert (
        int(args.datapoints) % 3 == 0 and int(args.datapoints) >= 3
    ), "Cannot build a triangle with less than 3 points."
