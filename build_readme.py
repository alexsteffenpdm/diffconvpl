...
def get_generator_help_text():
    ...
    for entry in os.listdir(GENERATOR_PATH):
        if ".py" in entry and entry != "base.py":
            try:
                p = check_output(
                    ["python", f"{os.path.join(GENERATOR_PATH,entry)}", "--help"]
                )
...
def replace_paths(content: str, **kwargs):
    for key, value in kwargs["paths"].items():
        try:
            ...
            raise ValueError(
                f"Could not rebuild 'README.md'. Placeholder {key} was not found in 'readme_content.txt' at location {str(README_META_PATH)}"
            )
    return content
